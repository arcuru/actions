...
- `cargo-update` / `flake-update` / `actions-update` need a fine-grained `PAT_TOKEN` with `contents:write` + `pull-requests:write` on the calling repo. `update-hold` needs one too, but only `pull-requests:write` — it removes a label rather than pushing. Store it as either a repo secret or as an environment secret scoped to the env passed via the `environment:` input — both work with `secrets: inherit`, but an env-scoped secret only resolves if the caller actually passes that `environment:`. (The default `GITHUB_TOKEN` can't trigger downstream workflow runs, which would silently break the deps-hold check.)
